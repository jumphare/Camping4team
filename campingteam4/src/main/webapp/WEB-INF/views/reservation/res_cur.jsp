...
<table>
<tr>
	<th>번호</th><th>예약일</th><th>ID</th><th>장소</th><th>자리</th><th>시작</th><th>종료</th><th>인원</th><th>금액</th><th>상태</th>
</tr>
...
<tr>
	...
	<td>${fn:substring(res.end_date,0,10)} <%-- ${res.end_date } --%></td>
	<td>${res.num }</td> <td>${res.price }</td>
	<td><c:if test="${res.payment eq '0'}" >결제대기</c:if>
		 <c:if test="${res.payment eq '1'}" >예약완료</c:if>
		 <c:if test="${res.payment eq '2'}" >예약취소</c:if></td>
</tr>
</c:forEach>
</c:if>
</table>
